...
reset-btn-tooltip = Aktuális kiválasztások visszaállítása...
...
dot-tooltip = DNS-lekérdezések titkosítása TLS-sel a jobb adatvédelem érdekében (szerver támogatás szükséges)
test-latency = Kiválasztott szerver késleltetésének tesztelése
test-latency-tooltip = Hálózati késleltetés mérése a kiválasztott DNS-szerverhez
best-server = Legjobb szerver kiválasztása késleltetés alapján
best-server-tooltip = Alap DNS-szerverek tesztelése (szűrő változatok nélkül) és a leggyorsabb kiválasztása
latency-result = {""}
server-info = {""}
latency-testing = tesztelés...
latency-timeout = időtúllépés
latency-no-result = egyetlen szerver sem válaszolt
dns-check-hint = Alkalmazás után ellenőrizd a DNS-szolgáltatód a következő oldalon
dns-server-changed = A DNS szerver sikeresen megváltozott!
dns-server-failed = A DNS szerver beállítása sikertelen!
dns-server-reset = A DNS szerver visszaállítva!
dns-server-reset-failed = A DNS szerver visszaállítása sikertelen!
...
desktop-on-handheld-error = Az asztali kiadást próbálod telepíteni egy kézi eszközre. Kérjük, használd a Handheld kiadást a megfelelő támogatáshoz ezen a hardveren
