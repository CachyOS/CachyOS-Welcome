...
apply = Aplicar
reset = Restablecer
enable-encrypted-dns = Activar DNS sobre {$protocol} ({$abbr})
blocky-install-failed = ¡Error al instalar Blocky para el soporte de {$mode}!
...
custom-dns-ip = Direcciones {$version} (separadas por comas):
...
dns-check-hint = Después de aplicar, verifique su proveedor de DNS en {$dnscheck_url}
