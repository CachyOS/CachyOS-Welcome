...
about-dialog-comments = CachyOS のようこそ画面

# Tweaks page
tweaks = チューニング
fixes = メンテナンス
applications = アプリケーション
removed-db-lock = Pacman の DB ロックを解除しました。
lock-doesnt-exist = Pacman のDB ロックは存在しません。
orphans-not-found = 孤立パッケージ (orphans) が見つかりませんでした。
...
dns-server-reset-failed = DNS サーバーのリセットに失敗しました。
winboat-install-failed = Winboat のインストールに失敗しました。

# Tweaks page (tweaks)
tweak-enabled-title = {$tweak} を有効化
tweak-psd-tooltip = ブラウザのプロファイルで RAM を利用します (高速化、ディスクの消耗を抑制)
tweak-oomd-tooltip = メモリ不足時にプロセスを事前に終了させ、フリーズを防止します
tweak-bpftune-tooltip = ネットワーク設定を自動的に最適化します
tweak-bluetooth-tooltip = Bluetooth デバイス (マウス、オーディオ等) のサポートを有効にします
tweak-ananicycpp-tooltip = プロセスの優先度を自動調整し、システムの応答性を向上させます
tweak-cachyupdate-tooltip = トレイに更新通知を表示します

# Tweaks page (fixes)
remove-lock-title = DB ロックを解除
reinstall-title = 全パッケージを再インストール
reset-keyrings-title = キーリングをリセット
update-system-title = システム更新
remove-orphans-title = 孤立パッケージ (orphans) を削除
...
# Main Page (body)
offline-error = インターネットに接続されていないため、オンラインでのインストールを開始できません
