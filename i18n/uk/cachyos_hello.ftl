...
apply = Застосувати
reset = Скинути
enable-encrypted-dns = Увімкнути DNS через {$protocol} ({$abbr})
blocky-install-failed = Не вдалося встановити blocky для підтримки {$mode}!
...
custom-dns-ip = {$version}-адреси (через кому):
...
dns-check-hint = Після застосування перевірте вашого DNS-провайдера на {$dnscheck_url}
